...
// ==========================================
// GLOBALS & STATE INITIALIZATION (Pulled to root for iframe stability)
// ==========================================
...
// FIXED: Clock instantiation moved to root scope so animate() can safely query it
...
function playSynthPluck(time, isCollect = false) {
    ...
    if (isCollect) {
        osc.type = 'sine'; osc.frequency.setValueAtTime(pentatonicScale[Math.floor(Math.random() * pentatonicScale.length)] * 4.0, time);
        gain.gain.setValueAtTime(0.25, time); gain.gain.exponentialRampToValueAtTime(0.001, time + 0.6);
    } else {
        osc.type = 'triangle'; osc.frequency.setValueAtTime(pentatonicScale[Math.floor(Math.random() * pentatonicScale.length)] * 0.5, time);
...
// Wait for DOM to ensure canvas element exists safely inside the iframe container
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Build Player Dragonfly Mesh
    const playerGroup = new THREE.Group();
    playerGroup.position.set(0, 5, 0);
    scene.add(playerGroup);

    const bodyGeo = new THREE.CylinderGeometry(0.4, 0.2, 2.8, 12); bodyGeo.rotateX(Math.PI / 2);
    const bodyMesh = new THREE.Mesh(bodyGeo, brassMaterial); bodyMesh.castShadow = true;
    ...
    wingOffsets.forEach(offset => {
        ...
        const joint = new THREE.Mesh(new THREE.CylinderGeometry(0.15, 0.15, 0.1, 8), brassMaterial); joint.rotation.z = Math.PI/2; pivot.add(joint);
        const wingFrameGeo = new THREE.BoxGeometry(2.6, 0.03, 0.5); wingFrameGeo.translate(1.3 * offset.signX, 0, 0);
        const frame = new THREE.Mesh(wingFrameGeo, brassMaterial); frame.castShadow = true;
        const wingGlassGeo = new THREE.BoxGeometry(2.4, 0.01, 0.4); wingGlassGeo.translate(1.3 * offset.signX, 0, 0);
        const glass = new THREE.Mesh(wingGlassGeo, new THREE.MeshPhysicalMaterial({ color: 0x88ccaa, transparent: true, opacity: 0.6, roughness: 0.1, transmission: 0.8, iridescence: 1.0 }));
        pivot.add(frame, glass);
    });

    const tailSegments = [];
    for (let i = 0; i < 6; i++) {
        const tailGeo = new THREE.CylinderGeometry(0.18 - i * 0.025, 0.15 - i * 0.025, 0.5, 8); tailGeo.rotateX(Math.PI / 2);
        const seg = new THREE.Mesh(tailGeo, brassMaterial); seg.position.set(0, 0, 1.4 + i * 0.45);
        playerGroup.add(seg); tailSegments.push(seg);
    }

    const vialOuterGeo = new THREE.CylinderGeometry(0.18, 0.18, 0.8, 12); vialOuterGeo.rotateX(Math.PI / 2);
    ...
    const vialInnerGeo = new THREE.CylinderGeometry(0.15, 0.15, 0.72, 8); vialInnerGeo.rotateX(Math.PI / 2);
    ...
    // Geometries & Materials for World Procedural Generation
    ...
    const honeyMat = new THREE.ShaderMaterial({
        uniforms: globalUniforms, transparent: true, side: THREE.FrontSide,
    ...
    const arcMat = new THREE.ShaderMaterial({
        ...
        fragmentShader: `uniform float uTime; varying vec2 vUv; void main() { float t = uTime * 8.0; float wave = sin(vUv.x * 15.0 + t) * 0.15 + sin(vUv.x * 30.0 - t * 1.2) * 0.05; float dist = abs(vUv.y - 0.5 + wave); float glow = 0.015 / (dist + 0.005); gl_FragColor = vec4(vec3(0.1, 0.8, 1.0) * glow, clamp(glow, 0.0, 1.0)); }`
    });
    const teslaArcGeo = new THREE.CylinderGeometry(0.4, 0.4, 20, 8); teslaArcGeo.rotateZ(Math.PI/2);

    const furnaceMat = new THREE.ShaderMaterial({
        uniforms: globalUniforms,
        vertexShader: `varying vec2 vUv; void main() { vUv = uv; gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }`,
        fragmentShader: `uniform float uTime; varying vec2 vUv; void main() { float n = sin(vUv.x*15.0 + uTime*2.0) * sin(vUv.y*15.0 - uTime*1.5); vec3 col = mix(vec3(0.8, 0.1, 0.0), vec3(1.0, 0.6, 0.0), smoothstep(-0.5, 0.5, n)); gl_FragColor = vec4(col, 1.0); }`
    });

    const combGroupGeo = new THREE.Group();
    const singleHexGeo = new THREE.CylinderGeometry(1.2, 1.2, 0.5, 6); singleHexGeo.rotateX(Math.PI/2); singleHexGeo.rotateZ(Math.PI/6);
    ...
    hexCoords.forEach(c => { const hex = new THREE.Mesh(singleHexGeo, honeycombMat); hex.position.set(c[0], c[1], 0); combGroupGeo.add(hex); });
    ...
    const sawGeo = new THREE.ExtrudeGeometry(sawShape, {depth: 0.6, bevelEnabled: true, bevelThickness: 0.1, bevelSize:0.1}); sawGeo.translate(0, 0, -0.3);
    ...
    const ventBaseGeo = new THREE.BoxGeometry(4, 1.5, 4); const steamGeo = new THREE.CylinderGeometry(1.5, 3.5, 15, 8); steamGeo.translate(0, 7.5, 0);
    const teslaPillarGeo = new THREE.CylinderGeometry(0.8, 1.2, 15, 8); const insulatorGeo = new THREE.TorusGeometry(1.4, 0.4, 8, 16); insulatorGeo.rotateX(Math.PI/2);
    const cargoBoxGeo = new THREE.BoxGeometry(8, 6, 8);

    // Canvas Cockpit Hud Geometry attachment
    ...
    hudMesh = new THREE.Mesh(hudPanelGeo, hudPanelMat); hudMesh.position.set(0, -1.2, -3); hudMesh.rotation.x = -0.3;
    camera.add(hudMesh);
    ...
    for (let i = 0; i < NUM_CHUNKS; i++) {
        ...
        const archsData = [];
        if (Math.random() > 0.6) {
            const arch = new THREE.Mesh(archGeo, honeyMat); const offsetZ = (Math.random() - 0.5) * (TILE_SIZE * 0.8); arch.position.set(0, 0, offsetZ); chunk.add(arch); archsData.push({ offsetZ: offsetZ });
        }
    ...
    // Dust Matrix
    const dustParticles = new THREE.Points(dustGeo, dustMat); scene.add(dustParticles);

    // Particle execution pool
    const particlePool = [];
    for (let p = 0; p < particleCount; p++) {
        const pm = new THREE.Mesh(particleGeo, particleMat.clone()); pm.visible = false; scene.add(pm);
    ...
    function handleInput(delta) {
        ...
        const targetVelX = axisX * 22.0; const targetVelY = axisY * 22.0;
        ...
        if (gameState === 'PLAYING') {
            if (audioCtx) {
                while (nextNoteTime < audioCtx.currentTime + 0.1) {
                    playSynthPluck(nextNoteTime, false); nextNoteTime += 0.35 - (currentSpeed * 0.1); 
                }
            }
            ...
            if (invulnerabilityTime > 0) {
                ...
                if (invulnerabilityTime <= 0) {
                    ...
                    playerGroup.children.forEach(c => { if(c.material && c.material.emissive && c.geometry !== eyeGeo) c.material.emissive.setHex(0x000000); });
                }
            }
            ...
            levelTiles.forEach((tile, index) => {
                ...
                if (tile.userData.obstacles) { tile.userData.obstacles.forEach(obs => { if (invulnerabilityTime <= 0 && obs.logic(time, delta, playerGroup.position, difficultyFactor)) takeDamage(); }); }
                if (tile.userData.archs && invulnerabilityTime <= 0) {
                    tile.userData.archs.forEach(arch => {
                        if (Math.abs(playerGroup.position.z - (tile.position.z + arch.offsetZ)) < 2.0 && Math.abs(Math.hypot(playerGroup.position.x, playerGroup.position.y) - 12.0) < 1.8) takeDamage();
                    });
                }
                ...
                if (comb && comb.visible) {
                    ...
                    if (playerGroup.position.distanceTo(tempV2) < 3.5) { honeycombCount++; comb.visible = false; triggerSplinterBurst(tempV2, 0xffd700); if (audioCtx) playSynthPluck(audioCtx.currentTime, true); }
                }
            });
