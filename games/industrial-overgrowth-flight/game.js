...
const splash = document.getElementById('ui-start');
const end = document.getElementById('ui-gameover');
const scoreEl = document.getElementById('hud-dist');
const combsEl = document.getElementById('hud-combs');
...
// Forest: Highly Diverse Autumnal Foliage (Pines, Maples, Oaks)
...
// ------------------------------------------
// BRASS HONEYCOMB CLUSTERS
// ------------------------------------------
const combGroupGeo = new THREE.Group();
const singleHexGeo = new THREE.CylinderGeometry(1.2, 1.2, 0.5, 6);
singleHexGeo.rotateX(Math.PI/2);
singleHexGeo.rotateZ(Math.PI/6);
const honeycombMat = new THREE.MeshStandardMaterial({ color: 0xb5a642, emissive: 0x3a2e0a, roughness: 0.3, metalness: 0.8 });
const hexCoords = [[0,0], [0, 2.1], [1.8, 1.05], [-1.8, 1.05], [0, -2.1]];
hexCoords.forEach(c => {
    const hex = new THREE.Mesh(singleHexGeo, honeycombMat);
    hex.position.set(c[0], c[1], 0);
    combGroupGeo.add(hex);
});

// ------------------------------------------
// GLSL SHADERS & ADVANCED MATERIALS
// ------------------------------------------
...
class ArchCurve extends THREE.Curve {
    ...
    getPoint(t, optionalTarget = new THREE.Vector3()) {
        ...
        return optionalTarget.set(Math.cos(angle) * this.radius, Math.sin(angle) * this.radius, 0);
    }
}
const archGeo = new THREE.TubeGeometry(new ArchCurve(12), 24, 1.5, 10, false);
const honeyMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
        ...
        void main() {
            float noise = sin(vUv.x * 12.0 + uTime * 2.5) * sin(vUv.y * 12.0 - uTime * 3.5);
            vec3 baseColor = mix(vec3(0.7, 0.3, 0.0), vec3(1.0, 0.7, 0.0), smoothstep(-0.5, 0.5, noise));
            float fresnel = pow(1.0 - max(dot(vNormal, vec3(0.0, 0.0, 1.0)), 0.0), 2.5);
            if(sin(vUv.y * 35.0 - uTime * 4.0) * sin(vUv.x * 35.0) > 0.85) baseColor += vec3(0.4);
            gl_FragColor = vec4(baseColor + vec3(fresnel * 0.6), 0.85);
        }
    `
});

// Tesla Plasma Arc
const teslaArcGeo = new THREE.CylinderGeometry(0.4, 0.4, 20, 8);
teslaArcGeo.rotateZ(Math.PI/2);
const arcMat = new THREE.ShaderMaterial({
    ...
    fragmentShader: `
        ...
        void main() {
            float t = uTime * 8.0;
            float wave = sin(vUv.x * 15.0 + t) * 0.15 + sin(vUv.x * 30.0 - t * 1.2) * 0.05;
            float dist = abs(vUv.y - 0.5 + wave);
            float glow = 0.015 / (dist + 0.005);
            gl_FragColor = vec4(vec3(0.1, 0.8, 1.0) * glow, clamp(glow, 0.0, 1.0));
                }
            `
        });

// Furnace Magma Core (for Pistons & Vents)
const furnaceMat = new THREE.ShaderMaterial({
    uniforms: globalUniforms,
    vertexShader: `varying vec2 vUv; void main() { vUv = uv; gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0); }`,
    fragmentShader: `
        uniform float uTime; varying vec2 vUv;
        void main() {
            float n = sin(vUv.x*15.0 + uTime*2.0) * sin(vUv.y*15.0 - uTime*1.5);
            vec3 col = mix(vec3(0.8, 0.1, 0.0), vec3(1.0, 0.6, 0.0), smoothstep(-0.5, 0.5, n));
...
// ------------------------------------------
// HAZARD GEOMETRY CONSTRUCTION
// ------------------------------------------
// 0. Pendulum (Chain + Low-Hanging Spiked Ball closer to flight path)
const pendulumGroupGeo = new THREE.Group();
...
for(let i=0; i<18; i++) {
    ...
    pendulumGroupGeo.add(link);
}
const weight = new THREE.Mesh(new THREE.SphereGeometry(3.0, 16, 16), hazardMat);
weight.position.y = -15;
weight.castShadow = true;
pendulumGroupGeo.add(weight);
...
for(let i=0; i<=teethCount*2; i++) {
    ...
    const r = (i%2===0) ? 4.5 : 3.5;
    if(i===0) sawShape.moveTo(Math.cos(angle)*r, Math.sin(angle)*r);
    else sawShape.lineTo(Math.cos(angle)*r, Math.sin(angle)*r);
}
const sawHole = new THREE.Path();
sawHole.absarc(0, 0, 1.5, 0, Math.PI*2, false);
sawShape.holes.push(sawHole);
const sawGeo = new THREE.ExtrudeGeometry(sawShape, {depth: 0.6, bevelEnabled: true, bevelThickness: 0.1, bevelSize:0.1});
sawGeo.translate(0, 0, -0.3);

// 2. Piston
const pistonShaftGeo = new THREE.CylinderGeometry(1.0, 1.0, 20, 8);
const pistonHeadGeo = new THREE.BoxGeometry(6, 2, 6);
const furnaceBaseGeo = new THREE.CylinderGeometry(4.0, 4.0, 2.0, 16);

// 3. Steam Vent
const ventBaseGeo = new THREE.BoxGeometry(4, 1.5, 4);
const steamGeo = new THREE.CylinderGeometry(1.5, 3.5, 15, 8);
steamGeo.translate(0, 7.5, 0); 
const steamMat = new THREE.MeshBasicMaterial({ color: 0xaaddcc, transparent: true, opacity: 0.4, depthWrite: false });

// 4. Tesla Coils
const teslaPillarGeo = new THREE.CylinderGeometry(0.8, 1.2, 15, 8);
const insulatorGeo = new THREE.TorusGeometry(1.4, 0.4, 8, 16);
insulatorGeo.rotateX(Math.PI/2);

// 5. Cargo Box
const cargoBoxGeo = new THREE.BoxGeometry(8, 6, 8);

// ==========================================
// HELPERS (buildPillar & drawRoundRect)
// ==========================================
function buildPillar(x, height) {
    const p = new THREE.Group(); 
    p.position.set(x, 0, 0);
    
    const colGeo = new THREE.CylinderGeometry(0.8, 1.2, height, 8);
    const col = new THREE.Mesh(colGeo, rustMat); 
    col.position.y = height / 2;
    p.add(col);

    const numInsulators = Math.floor(height / 4);
    for(let j = 0; j < numInsulators; j++) {
        const r = new THREE.Mesh(insulatorGeo, metalMat);
        r.position.y = height - 1.5 - (j * 1.5);
        p.add(r);
    }
    return p;
}
...
function renderDynamicHUD() {
    ...
    hudCtx.fillStyle = 'rgba(239, 227, 181, 0.9)';
    drawRoundRect(hudCtx, 0, 0, 512, 128, 16); 
    hudCtx.fill();
    
    hudCtx.strokeStyle = '#2d3c28';
    hudCtx.lineWidth = 4;
    hudCtx.stroke();
    hudCtx.strokeStyle = 'rgba(45, 60, 40, 0.5)';
    hudCtx.lineWidth = 2;
    hudCtx.strokeRect(8, 8, 496, 112);

    hudCtx.fillStyle = '#2d3c28';
    hudCtx.font = 'bold 16px Georgia';
    hudCtx.fillText("INTEGRITY", 40, 45);
    hudCtx.font = 'bold 24px Arial';
    hudCtx.fillStyle = '#cc0000';
    hudCtx.fillText("❤️".repeat(lives) + "🖤".repeat(3-lives), 40, 75);

    hudCtx.fillStyle = '#2d3c28';
    hudCtx.font = 'bold 16px Georgia';
    hudCtx.fillText("ALTITUDE", 210, 45);
    hudCtx.font = 'bold 24px monospace';
    hudCtx.fillStyle = playerGroup.position.y > dynamicMaxSafeY ? '#cc0000' : '#1e3816';
    hudCtx.fillText(`${Math.max(0, playerGroup.position.y * 10).toFixed(0)}M`, 210, 75);

    hudCtx.font = 'bold 16px Georgia';
    hudCtx.fillStyle = '#2d3c28';
    hudCtx.fillText("HONEYCOMBS", 340, 45);
    hudCtx.font = 'bold 24px monospace';
    hudCtx.fillStyle = '#1e3816';
    hudCtx.fillText(`${honeycombCount}`, 340, 75);
...
function resetGame() {
    distanceTraveled = 0;
    honeycombCount = 0;
    lastUpgradeMilestone = 0;
    nextUpgradeDistance = 500;
    currentSpeed = 0.5;
    targetSpeed = 0.55;
    maxSpeedBonus = 0.0;
    lives = 3;
    ...
    lastAltitudeDamageTime = 0;
    
    isBoosting = false;
    boostTimer = 0;
    boostCooldown = 0;
    boostCooldownModifier = 1.0;
...
function takeDamage() {
    ...
    lives--;
    triggerSplinterBurst(playerGroup.position, 0xff2222);
    playImpactSound();
    
    currentSpeed = 0.15; 
    targetSpeed = 0.55 + maxSpeedBonus;
    invulnerabilityTime = 2.0; 
    
    playerGroup.children.forEach(c => {
        if(c.material && c.material.emissive) c.material.emissive.setHex(0x550000);
    });
...
function handleInput(delta) {
    let axisX = 0;
    let axisY = 0;
    let engineBoost = keys[' '];
    let triggerOverdrive = keys['Shift'];
    ...
    if (gp) {
        if (Math.abs(gp.axes[0]) > 0.15) axisX = gp.axes[0];
        if (Math.abs(gp.axes[1]) > 0.15) axisY = -gp.axes[1]; 
        if (gp.buttons[0] && gp.buttons[0].pressed) engineBoost = true;
        const btnX = gp.buttons[2] && gp.buttons[2].pressed;
        if (btnX && !lastButtonStateX) triggerOverdrive = true;
        lastButtonStateX = btnX;
    }

    if (axisX === 0 && axisY === 0) {
        if (keys.a || keys.ArrowLeft) axisX = -1;
        if (keys.d || keys.ArrowRight) axisX = 1;
        if (keys.w || keys.ArrowUp) axisY = 1;
        if (keys.s || keys.ArrowDown) axisY = -1;
    }
    
    if (axisX === 0 && axisY === 0 && (pointerX !== 0 || pointerY !== 0)) {
        axisX = pointerX;
        axisY = pointerY;
    }

    let localMaxSpeed = 0.55 + maxSpeedBonus;
    if (engineBoost) {
        localMaxSpeed = 0.95 + maxSpeedBonus;
    }

    if (triggerOverdrive && boostCooldown <= 0) {
        isBoosting = true;
        boostTimer = 1.0; 
        boostCooldown = 3.5 * boostCooldownModifier;
        playDodgeSound();
        triggerSplinterBurst(playerGroup.position, 0xffbb44);
    }
    ...
    if (isBoosting) {
        boostTimer -= delta;
        targetSpeed = 2.4 + maxSpeedBonus; 
        if (boostTimer <= 0) {
            isBoosting = false;
        }
    } else {
        targetSpeed = localMaxSpeed;
    }

    const dampeningFactor = isBoosting ? 1.0 : 2.5; 
    const targetVelX = axisX * 22.0;
    const targetVelY = axisY * 22.0;

    playerVelocity.x += (targetVelX - playerVelocity.x) * dampeningFactor * delta;
    playerVelocity.y += (targetVelY - playerVelocity.y) * dampeningFactor * delta;
    ...
    if (isBoosting && Math.random() > 0.4) {
        tempV1.copy(playerGroup.position);
        tempV1.z += 1.8; 
        triggerSplinterBurst(tempV1, 0xff7700);
    }

    const deltaX = playerGroup.position.x - prevPlayerX;
    const goalRoll = -playerVelocity.x * 0.05; 
    const goalPitch = playerVelocity.y * 0.04;

    playerGroup.rotation.z += (goalRoll - playerGroup.rotation.z) * 6.0 * delta;
    playerGroup.rotation.x += (goalPitch - playerGroup.rotation.x) * 6.0 * delta;
}
