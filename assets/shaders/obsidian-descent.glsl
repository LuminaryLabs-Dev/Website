...
void mainImage(out vec4 O,vec2 fragCoord){vec2 uv=(2.*fragCoord-iResolution.xy)/iResolution.y;vec3 ro=vec3(0,0,iTime*4.),rd=normalize(vec3(uv,1.2));rd.xy*=ROT(sin(iTime*.2)*.4);float t=0.,d;vec3 p;for(int i=0;i<110;i++){p=ro+rd*t;d=map(p);if(d<.001||t>40.)break;t+=d*.5;}vec3 col=vec3(.01,.01,.015);if(t<40.){vec3 n=normal(p);float z=max(0.,dot(n,-rd));float nz=noise(p*.8-vec3(0,0,iTime*.4));float bands=abs(fract(nz*8.)-.5);float crease=(1.-smoothstep(0.,.05,bands));float bleed=(1.-smoothstep(0.,.3,bands));vec3 neon=.5+.5*cos(6.28318*(nz*1.5+iTime*.2+vec3(0.,.333,.667)));col=vec3(.04,.05,.07)*z*(.85+.3*nz)+vec3(.1,.15,.2)*pow(1.-z,4.)+pow(z,32.+16.*nz)*.5;col+=neon*crease*3.+neon*bleed*.5;col=mix(col,vec3(.01),smoothstep(15.,40.,t));}O=vec4(pow(col,vec3(.7)),1.);}
