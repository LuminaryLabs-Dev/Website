...
void mainImage(out vec4 color, in vec2 fragCoord) {
  ...
  for(int i=0;i<10;i++) {
    ...
    for(int s=0;s<2;s++) {
      ...
      float slope=-0.19+side*shift;
      hitPlane(ro,rd,vec3(side,0,-slope),4.55+side*bend(start)-slope*start,start,start+4.5,3.0,t,n,material);
      slope=0.19+side*shift;
      hitPlane(ro,rd,vec3(side,0,-slope),3.695+side*(bend(start)+shift*4.5)-slope*(start+4.5),start+4.5,start+BAY,3.0,t,n,material);
      ...
      slope=0.19+side*shift;
      hitPlane(ro,rd,vec3(side,0.85,-slope),3.695+side*(bend(start)+shift*4.5)+0.85*4.65-slope*(start+4.5),start+4.5,start+BAY,4.0,t,n,material);
    }
  }
  vec3 p=ro+rd*t;
  ...
  if(material==3.0 || material==4.0) {
    ...
    col=vec3(0.29,0.34,0.38)*tex*facing;
