...
vec2 scene(vec3 p){
 p.xy-=route(p.z);
 float radial=length(vec2(p.x,p.y+.8));
 float d=100.;
 for(int i=0;i<3;i++){
  float f=pow(2.,float(i));
  float z=mod(p.z+3./f,6./f)-3./f;
  float radius=3.35+float(i)*.57;
  float arch=length(vec2(radial-radius,z))-.19/f;
  d=min(d,arch);
 }
 // Broad stepped mineral terraces outside the passage.
 float angle=atan(p.y,p.x);
 float steps=floor((p.y+6.)*2.)*.07;
 float wall=5.0+steps-radial+.16*cos(angle*8.);
 float terrace=p.y+2.35;
 for(int i=0;i<6;i++){
  float level=float(i);
  terrace=min(terrace,box3(vec3(abs(p.x)-1.9-level*.48,p.y+2.35-level*.12,p.z),vec3(.245,.08+level*.12,10000.)));
 }
 return closer(closer(vec2(d,2.),vec2(wall,1.)),vec2(terrace,3.));
}

vec3 palette(float id,vec3 p,vec3 n,vec3 rd,float aa){
 vec3 base=vec3(.32,.26,.23),accent=vec3(.95,.60,.30);
 if(id>2.5)base=vec3(.20,.23,.25);
 ...
 float spec=pow(max(dot(reflect(rd,n),normalize(vec3(.2,.8,-.5))),0.),id>2.5?56.:32.);
 col+=vec3(.40,.36,.30)*spec;
 col+=vec3(.10,.13,.16)*rim*max(n.y,0.);
 return col;
}
